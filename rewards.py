...
def semantic_reward_func(prompts, completions, **kwargs) -> list[float]:
    ...
    for prompt, completion in zip(prompts, completions):
        ...
        if answer_match:
            ...
            pred_answer = " ".join(words[:2])
            pred_answer = re.sub(r'[^a-zA-Z0-9 ]', '', pred_answer) 
...
def format_reward(completions, **kwargs) -> list[float]:
    ...
    for completion in completions:
        ...
        answer_match = re.search(r"<answer>(.*?)(?:</answer>|$)", response, flags=re.IGNORECASE)
        is_valid_answer_length = False
        if answer_match:
            answer = answer_match.group(1).strip()
            answer = re.sub(r'<[^>]+>', '', answer)
            word_count = len(answer.split())
            is_valid_answer_length = 1 <= word_count <= 2
        ...
        has_other_tags = bool(re.search(r'<\s*(?!/?(thought|answer)\b)[a-zA-Z][^>]*>', response, flags=re.IGNORECASE))
        ...
        print(f"- Is Valid Answer length: {is_valid_answer_length}")
